...
2. 通常情况下相似度计算可能需要大量数据支持，对于稀疏数据需要对数据规模进行置信度加权，常用加权方法可以是![4](../images/alg/svd++/n1.png)。
3. 用户对某物品打分公式为![5](../images/alg/svd++/n2.png)，其中bui为基准推荐模型，计算公式为![6](../images/alg/svd++/n3.png)，即全局偏置+用户偏置+物品偏置；Ski;u表示在用户关注的物品集合中和物品i最相似的前K个。
4. 另外为了进一步刻画效果，可以通过下面方式进行改进![7](../images/alg/svd++/n4.png)更和用户相关。
